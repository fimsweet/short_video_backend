...
export class VideoProcessorService implements OnModuleInit, OnModuleDestroy {
  ...
  // ============================================
  // Luu reference t?i FFmpeg process dang ch?y d? có th? kill khi shutdown
  ...
  async onModuleInit() {
    // T?o thu m?c processed_videos n?u chua có
  ...
  // ============================================
  // Khi K8s g?i SIGTERM, pod có 30s (default) d? cleanup
  // Worker c?n:
  // 1. Ng?ng nh?n job m?i
  // 2. Ð?i job hi?n t?i hoàn thành
  // 3. Ðóng connection
  // ============================================
  async onModuleDestroy() {
    ...
    // Cancel consumer d? không nh?n job m?i
    if (this.channel) {
      try {
    ...
    // Ð?i job hi?n t?i hoàn thành (max 60s)
    const maxWait = 60;
    let waited = 0;
  ...
  private async startWorker(): Promise<void> {
    ...
    try {
      ...
      // ============================================
      // Error handlers d? tránh unhandled error crash
      // ============================================
  ...
  private async setupConsumer(channel: amqp.Channel, dlqName: string, dlqEnabled: boolean): Promise<void> {
    ...
    // ============================================
    // KHÔNG dùng os.cpus() vì trong Docker/K8s nó tr? v? CPU c?a host node
    // Ví d?: Node có 64 cores nhung Pod gi?i h?n 1 core ? code th?y 64 ? crash
    //
    // BEST PRACTICE cho AWS/K8s:
    // - Set WORKER_CONCURRENCY=1 ho?c 2 trong Deployment YAML
    // - Scale b?ng s? lu?ng Pod (Horizontal Scaling) thay vì nhi?u job/Pod
    // - V?i KEDA: Queue d?y ? auto scale Pods
    //
    // FFmpeg là CPU-bound task, ch?y nhi?u job song song trên 1 Pod s?:
    // - Context switching liên t?c ? ch?m hon làm tu?n t?
    // - Nguy co OOM cao
    // ============================================
    ...
    // Log warning n?u concurrency > 2 trên production
    ...
    channel.consume(this.queueName, async (msg) => {
      if (msg !== null) {
        ...
        try {
          ...
          if (dlqEnabled) {
            // Job chuy?n vào DLQ d? admin review
            ...
            // N?u video l?i (file h?ng, codec không h? tr?...), requeue s? t?o
            // infinite loop: Error -> Requeue -> Error -> Requeue...
            // 
            // Gi?i pháp: Ack message (xóa kh?i queue) và update DB = FAILED
            // Video b? m?t nhung h? th?ng không b? treo
            // ============================================
  ...
  private async processVideo(job: any): Promise<void> {
    ...
    try {
      ...
      // 1. L?y thông tin video t? database
      ...
      // 4. X? lý video b?ng FFmpeg (convert sang HLS)
      ...
      // 8. C?p nh?t database v?i aspect ratio và thumbnail
